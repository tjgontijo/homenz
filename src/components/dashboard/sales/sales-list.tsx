...
import type { LeadSale } from '@/lib/schema/lead-inbounds'
import { formatCurrencyBRL, formatDateTime } from '@/lib/utils/formatters'
  ...
  return (
    <div className="space-y-3">
      {sales.map((sale) => {
        ...
        const fallbackDescription = showFallback ? formatRawDescription(sale.raw_description) : ''
        ...
        return (
          ...
          <header className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              ...
              <p className="flex flex-wrap items-center gap-1 text-xs text-muted-foreground">
                <span>Registrada em {formatDateTime(sale.created_at)}</span>
          ...
          {sale.inbound && (
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              {renderTag('Status', sale.inbound.status)}
              {renderTag('Origem', sale.inbound.utm_source)}
              {renderTag('Meio', sale.inbound.utm_medium)}
              {renderTag('Campanha', sale.inbound.utm_campaign)}
              <span className="inline-flex items-center gap-1 rounded-full border border-muted-foreground/30 bg-muted/10 px-2 py-1">
                ...
                <span className="text-xs font-medium text-foreground">{formatDateTime(sale.inbound.created_at)}</span>
              </span>
            </div>
          )}
