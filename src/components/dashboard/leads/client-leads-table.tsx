...
import { LeadInboundsDialog } from '@/components/dashboard/inbounds/lead-inbounds-dialog';
...
type BooleanFilterKey = 'hasInbound' | 'hasSales' | 'hasMessages' | 'hasAudit';

const BOOLEAN_FILTER_OPTIONS: Array<{
  key: BooleanFilterKey;
  label: string;
}> = [
  { key: 'hasInbound', label: 'Inbounds' },
...
export default function ClientLeadsTable() {
  ...
  const activeBooleanFilters = React.useMemo<Record<BooleanFilterKey, boolean>>(() => {
    return {
      hasInbound: searchParams.get('hasInbound') === 'true',
  ...
  const handleOpenDialog = React.useCallback((lead: Lead, mode: 'inbounds' | 'sales' | 'messages' | 'audits') => {
    setDialog({ lead, mode });
  }, []);
  ...
  const columns = React.useMemo<ColumnDef<Lead>[]>(
    () => [
      ...
      {
        header: () => <span className="block text-center">Inbound</span>,
        accessorKey: 'hasInbound',
        size: 80,
        ...
        cell: ({ getValue, row }) => {
          ...
          return (
            <div className="flex justify-center">
              <StatusIconButton
                hasData={value}
                label="Ver inbounds do lead"
                onClick={() => handleOpenDialog(lead, 'inbounds')}
                activeIcon={Inbox}
                inactiveIcon={Inbox}
